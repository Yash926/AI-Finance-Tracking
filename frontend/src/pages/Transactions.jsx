...
export default function Transactions() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      if (filter.month && filter.year) {
        ...
        params.append('startDate', new Date(y, m - 1, 1).toISOString());
        params.append('endDate',   new Date(y, m, 0, 23, 59, 59).toISOString());
      }
  ...
  const handleSuccess = () => {
    setShowForm(false); setEditData(null); fetchTransactions();
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <button onClick={() => { setShowForm(true); setEditData(null); }} style={{
      ...
      <div style={{ ...cardStyle, marginBottom: '16px', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
        {[
          {
            ...
            options: [{ v: '', l: 'All Types' }, { v: 'income', l: 'Income' }, { v: 'expense', l: 'Expense' }],
          },
          {
            key: 'month', placeholder: 'All Months',
            options: [{ v: '', l: 'All Months' }, ...MONTHS.map((m, i) => ({ v: i + 1, l: m }))],
          },
          {
            key: 'year', placeholder: 'Year',
            options: [2023, 2024, 2025, 2026].map((y) => ({ v: y, l: y })),
          },
        ].map(({ key, options }) => (
          <select key={key}
            value={filter[key]}
        ...
        {filter.type && (
          <select
            ...
            onChange={(e) => { setFilter((p) => ({ ...p, category: e.target.value })); setPage(1); }}
        ...
        <button onClick={() => { setFilter({ type: '', category: '', month: '', year: new Date().getFullYear() }); setPage(1); }}
        ...
        <div style={{
          ...
          <div style={{ ...cardStyle, width: '100%', maxWidth: '480px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h5 style={{ color: '#e2e8f0', margin: 0, fontWeight: 600 }}>
            ...
            <TransactionForm editData={editData} onSuccess={handleSuccess}
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '20px' }}>
                {Array.from({ length: pages }, (_, i) => (
                  <button key={i} onClick={() => setPage(i + 1)} style={{
                    ...
                    background: page === i + 1 ? 'linear-gradient(135deg,#4361ee,#7209b7)' : 'rgba(255,255,255,0.06)',
                    color: page === i + 1 ? '#fff' : '#94a3b8', fontWeight: 600,
                  }}>
                    {i + 1}
                  </button>
                ))}
